...
export function Sidebar({ mobile }: SidebarProps) {
  ...
  return (
    <div className={clsx(
      ...
      mobile ? 'w-full' : 'w-64 border-r border-gray-200',
    )}>
      {/* Logo */}
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {navItems.map(({ to, icon: Icon, label }) => (
          <NavLink key={to} to={to} end={to === ROUTES.DASHBOARD} className={linkClass}
      ...
      <div className="px-3 py-3 border-t border-gray-100 space-y-1">
        ...
        <div className="flex items-center gap-3 px-3 py-2 mt-1">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-gray-900 truncate">{profile?.full_name || 'Użytkownik'}</p>
            <p className="text-xs text-gray-400 truncate">{profile?.email}</p>
          </div>
          <button onClick={handleLogout} className="text-gray-400 hover:text-gray-600 p-1 shrink-0" title="Wyloguj">
            <LogOut size={16} />
          </button>
        </div>
      </div>
